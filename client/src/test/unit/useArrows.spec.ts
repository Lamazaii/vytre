...
describe('generateArrowPath', () => {
  ...
  it('returns a simple line path for stroke endStyle (arrowheads drawn by canvas)', () => {
    const result = generateArrowPath({ x: 0, y: 0 }, { x: 100, y: 0 }, 'none', 'stroke', 2)
    // Arrowheads are drawn in handleArrowRender, not embedded in the path
    expect((result.match(/M/g) || []).length).toBe(1)
  })

  it('returns a simple line path for stroke startStyle (arrowheads drawn by canvas)', () => {
    const result = generateArrowPath({ x: 0, y: 0 }, { x: 100, y: 0 }, 'stroke', 'none', 2)
    expect((result.match(/M/g) || []).length).toBe(1)
  })

  it('returns a simple line path when both styles are stroke', () => {
  ...
  it('returns the same base path regardless of strokeWidth (size handled by canvas render)', () => {
    ...
    expect(thin).toMatch(/^M\s+0,0\s+L\s+100,0$/)
    expect(thick).toMatch(/^M\s+0,0\s+L\s+100,0$/)
  })

  it('path always starts at M 0,0 (offsets handled by canvas render)', () => {
    const result = generateArrowPath({ x: 0, y: 0 }, { x: 100, y: 0 }, 'stroke', 'none', 2)
    expect(result).toMatch(/^M\s+0,0/)
  })

  it('path always ends at full distance (offsets handled by canvas render)', () => {
    const result = generateArrowPath({ x: 0, y: 0 }, { x: 100, y: 0 }, 'none', 'stroke', 2)
    expect(result).toMatch(/L\s+100,0$/)
  })
})
...
describe('useArrows composable', () => {
  ...
  it('handleArrowMouseMove updates arrowStart when modifyingEnd is start (line 194)', () => {
    ...
    // Move — should update arrowStart in-place
